...
# Set process to high priority (for Windows)
...
con = sqlite3.connect("../databases/dec-24/all_datasets_labelled.db")
# multiple datasets
...
if machine_learning:
    ...
    if not_done:
        ...
        #loaded_model = joblib.load('models/logistic_regression_model.joblib')
        ...
        joblib.dump(svc, '../models/Suicide_SVM.pkl')
        ...
        # Save the model
        model_filename = '../models/naive_bayes_model.joblib'
        joblib.dump(mnb, model_filename)
        print(f"Model saved as {model_filename}")

        #To load this model later:
        #loaded_model = joblib.load('models/naive_bayes_model.joblib')
        print('\n')
        # Perform comprehensive misclassification analysis
        perform_misclassification_analysis(
            df_actual=df_actual,            # Actual test data
    ...
    #joblib.dump(ec, 'models/Suicide_Ensemble.pkl')
    # Save the model
    model_filename = '../models/Suicide_Ensemble.pkl'
    joblib.dump(ec, model_filename)
    print(f"Model saved as {model_filename}")

    #To load the saved Ensemble model:
    #loaded_model = joblib.load('models/Suicide_Ensemble.pkl')
    ...
    # Save the model
    model_filename = '../models/adaboost_random_forest_model.joblib'
...
# Define callbacks
checkpoint_callback = ModelCheckpoint(filepath="models/lstm-1-layer-best_model.h5", save_best_only=True, monitor="val_acc", mode="max", verbose=1)
...
#model = load_model('models/lstm-1-layer-best_model.h5')
...
checkpoint_callback = ModelCheckpoint(filepath="../models/lastm-2-layer-best_model.h5", save_best_only=True, monitor="val_acc", mode="max", verbose=1)
...
# Load the best model
#model = load_model('models/lastm-2-layer-best_model.h5')
...
checkpoint_callback = ModelCheckpoint(filepath="models/gru-best_model.h5", save_best_only=True, monitor="val_acc", mode="max", verbose=1)
...
# Load the GRU model
#model = load_model('models/gru-best_model.h5')
...
checkpoint_callback = ModelCheckpoint(filepath="../models/cnn+lastm-best_model.h5", save_best_only=True, monitor="val_acc", mode="max", verbose=1)
...
model = load_model('../models/cnn+lastm-best_model.h5')
...
model = load_model('../models/cnn+lastm-best_model.h5')
model_cl.save('../models/CNN+LSTM.h5')
...
# Save the model
model_filename = '../models/hybrid_cnn_lstm_gru_model.h5'
...
output = Dense(8, activation='softmax')(flatten_layer)
...
# Save the model
model_filename = '../models/lstm_with_attention_model.h5'
model.save(model_filename)
print(f"Model saved as {model_filename}")
...
predictor.save("../models/pre/")

#**Load Saved Model and Predict**
predictor1 = ktrain.load_predictor('/pre/')
...
model.save_pretrained('../models/roberta_text_classifier')
tokenizer.save_pretrained('../models/roberta_text_classifier')
print("Model and tokenizer saved successfully.")

# Load the model and tokenizer
#loaded_tokenizer = RobertaTokenizer.from_pretrained('models/roberta_text_classifier')
#loaded_model = TFRobertaForSequenceClassification.from_pretrained('models/roberta_text_classifier')
...
# Load the model and tokenizer
#loaded_tokenizer = DistilBertTokenizer.from_pretrained('models/distilbert_text_classifier')
#loaded_model = TFDistilBertForSequenceClassification.from_pretrained('models/distilbert_text_classifier')
...
anomaly_df.to_csv('../output/anomaly_detection_results.csv', index=False)
...
incorrect_df.to_csv('../output/incorrect_classifications.csv', index=False)
